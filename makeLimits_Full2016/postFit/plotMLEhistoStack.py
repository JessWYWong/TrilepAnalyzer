...
# dataFile = "/user_data/rsyarif/optimization_reMiniAOD_PRv9_FRv30CR2_newRunH_correctedMuTrkSF_AllSys_2017_7_14/lep1Pt0_jetPt0_MET20_NJets3_NBJets1_HT0_ST0_mllOS20/Shape_accurateLHESys_FRsysMar28_newSigSF_AsymmFRsys/templates_HTrebinned_TTM1000_bW0p5_tZ0p25_tH0p25_35p867fb.root"
...
#set data histo and prefit histos
hdata={}
htop_prefit = {}
hewk_prefit = {}
hddbkg_prefit = {}
for cat in category:
	...
	hewk_prefit[cat] = rFile.Get("triLep"+cat+"__ewk")
	htop_prefit[cat] = rFile.Get("triLep"+cat+"__top")
	hddbkg_prefit[cat] = rFile.Get("triLep"+cat+"__ddbkg")
...
# 		hddbkg[cat].SetBinError(ibin,hddbkg_tmp[cat].GetBinError(ibin))
		...
		#get stat error from prefit templates
		top_prefit_percentErr =0.0
		ewk_prefit_percentErr=0.0
		ddbkg_prefit_percentErr=0.0
		if(htop_prefit[cat].GetBinContent(ibin)!=0):top_prefit_percentErr = htop_prefit[cat].GetBinError(ibin)/htop_prefit[cat].GetBinContent(ibin)
		if(hewk_prefit[cat].GetBinContent(ibin)!=0):ewk_prefit_percentErr = hewk_prefit[cat].GetBinError(ibin)/hewk_prefit[cat].GetBinContent(ibin)
		if(hddbkg_prefit[cat].GetBinContent(ibin)!=0):ddbkg_prefit_percentErr = hddbkg_prefit[cat].GetBinError(ibin)/hddbkg_prefit[cat].GetBinContent(ibin)
		htop[cat].SetBinError(ibin, top_prefit_percentErr * htop[cat].GetBinContent(ibin))
		hewk[cat].SetBinError(ibin, ewk_prefit_percentErr * hewk[cat].GetBinContent(ibin))
		hddbkg[cat].SetBinError(ibin, ddbkg_prefit_percentErr * hddbkg[cat].GetBinContent(ibin))
...
# 					hddbkgSys[cat][sys+PM].SetBinError(ibin,0)
