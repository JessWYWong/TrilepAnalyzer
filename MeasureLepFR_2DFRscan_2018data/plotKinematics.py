...
plotList = ['lepPt','lepPtRebinned','lepPtRebinnedv1','lepPtRebinnedv3']
	...
	for muFRindex in xrange(loop):
		for cat in catList:
			...
			try: hQCD = RFile.Get(histPrefix+'__qcd').Clone()
				...
				print "Skipping QCD....."
				pass
			try: hDDBKG = RFile.Get(histPrefix+'_ddbkgMuFR'+str(muFRindex)).Clone()
			except: 
				print "There is no DDBKG MuFR"+str(muFRindex)+"!!!!!!!!"
				print "Skipping DDBKG MuFR"+str(muFRindex)+"....."
				pass
			...
			try: print discriminant,cat, "DDBKG MuFR"+str(muFRindex)+"", hDDBKG.Integral()
			except: pass
...
#				normByBinWidth(hsig2)
				...
				for sys in systematicList:
					for ud in ['minus','plus']:
						...
						try: systHists['ddbkg'+sys+ud] = RFile.Get(histPrefix+'__ddbkg__'+sys+'__'+ud).Clone()
							...
							print "Skipping",sys,"for DDBKG MuFR"+str(muFRindex)
							pass
			...
			bkgHTgerr = totBkgTemp3[cat].Clone()
...
#			leg.AddEntry(hsig3,sigleg+' #rightarrow tHtH'+scaleFact1Str,"l")
	...
	#			else: pullLegend.AddEntry(pullUncBandTot , "Bkg stat. #oplus syst." , "f")
			...
			if doOneBand:
				...
				c1.SaveAs(savePrefix+"_totBand"+"_ddbkg_MuFR"+str(muFRindex)+".png")
				#c1.SaveAs(savePrefix+"_totBand.root")
				#c1.SaveAs(savePrefix+"totBand.C")
			else:
	# 			c1.SaveAs(savePrefix+".pdf")
				c1.SaveAs(savePrefix+"_ddbkg_MuFR"+str(muFRindex)+".png")
