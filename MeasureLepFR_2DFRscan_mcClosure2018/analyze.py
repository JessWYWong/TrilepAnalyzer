...
#	'''
	...
	if 'MatrixBkgLTT' in process: cut+=' && isLTT==1'
	if 'MatrixBkgTLL' in process: cut+=' && isTLL==1'
...
#End -- Method if isTTT exist
	...
	if 'muFR0elFR0' in process: print "Applying Cuts: ", cut
	...
	if doAllSys or doDDBKGsys:
		if doDDBKGsys and 'MatrixBkg' in process: 	
...
# 				hists[discriminantName+'btagDown_'+lumiStr+'fb_'+category+'_'+process]  = R.TH1D(discriminantName+'btagDown_'+lumiStr+'fb_'+category+'_'+process,xAxisLabel,len(xbins)-1,xbins)
	...
	#Determins weights ---> #NOT APPLYING SF and THIgns!!
	if ('MatrixBkg' in process):
