...
//   h->GetZaxis()->SetTitle("#bar{#chi^{2}}");
  ...
  Double_t x = h->GetXaxis()->GetBinCenter( ix );//h->GetXaxis()->FindBin(xp) );
  Double_t y = h->GetYaxis()->GetBinCenter( iy );//h->GetYaxis()->FindBin(yp) );
  
  Double_t minChiSq = h->GetBinContent(ix,iy);//h->GetXaxis()->FindBin(xp),h->GetYaxis()->FindBin(yp));
...
void DrawConstChiSqContour(TString dir ="FRv1_kinematics_FWLJMET102X_3lep2017_062019_wiwong_step1hadds_step2FR_kinematics_FWLJMET102X_3lep2017_062019_wiwong_step1hadds_step2_2019_8_16"){
  ...
  float muFR = 0.11;//0.13;//0.15 ;
  float elFR = 0.08;//0.10 ;
