...
//   h->GetZaxis()->SetTitle("#bar{#chi^{2}}");
  ...
  double minRange = 0.0;
  double maxRange = 0.51;
  
  //bin = 1 is x = 0.0
  //std::cout<< h->GetXaxis()->GetBinLowEdge(1)<<";"<<h->GetNbinsX()<<h->GetXaxis()->GetBinLowEdge(h->GetNbinsX())<<endl;
  Int_t ix,iy,iz;
  h->GetMinimumBin(ix,iy,iz);

  Double_t x = h->GetXaxis()->GetBinCenter( ix );//h->GetXaxis()->FindBin(xp)+1 );
  Double_t y = h->GetYaxis()->GetBinCenter( iy );//h->GetYaxis()->FindBin(yp)+1 );

  std::cout<< "Minimum Bin " << ix<<","<<iy << " ; " << h->GetBinContent(ix,iy)<< " ; "<< h->GetXaxis()->GetBinLowEdge(ix)<<","<< h->GetYaxis()->GetBinLowEdge(iy) <<std::endl;
 
  Double_t minChiSq = h->GetBinContent(ix,iy);//h->GetXaxis()->FindBin(xp)+1,h->GetYaxis()->FindBin(yp)+1);
  //std::cout << "minChiSq = " << minChiSq << " at x="<<xp<<" and y="<<yp<<std::endl;
  Double_t chiSq = 0;
  ...
  for(int xbin=1; xbin <= h->GetNbinsX(); xbin++){
  	for(int ybin=1; ybin <= h->GetNbinsY(); ybin++){
  		...
  		if(fabs(minChiSq-chiSq) <= 2.30){ // && minChiSq!=chiSq){
  			std::cout << "chiSq (" << xbin<<", "<< ybin<< ") = " << chiSq << " --> |chiSq-minChiSq| = " << fabs(minChiSq-chiSq) <<" < 2.30"<<std::endl;	
  ...
  Double_t xmin = h->GetXaxis()->GetBinLowEdge(h->GetXaxis()->FindBin(minRange) );//Center( h->GetXaxis()->FindBin(minRange) );
  Double_t ymin = h->GetYaxis()->GetBinLowEdge(h->GetYaxis()->FindBin(minRange) );//Center( h->GetYaxis()->FindBin(minRange) );
  Double_t xmax = h->GetXaxis()->GetBinUpEdge( h->GetXaxis()->FindBin(maxRange) );//Center( h->GetXaxis()->FindBin(maxRange) );
  Double_t ymax = h->GetYaxis()->GetBinUpEdge( h->GetYaxis()->FindBin(maxRange) );//Center( h->GetYaxis()->FindBin(maxRange) );
...
//   h->GetZaxis()->SetTitle("#bar{#chi^{2}}");
  ...
  for(int xbin=1; xbin <= h->GetNbinsX(); xbin++){
  	for(int ybin=1; ybin <= h->GetNbinsY(); ybin++){
		...
		//std::cout << "chiSq (" << xbin<<", "<< ybin<< ") = " << chiSq << " --> prob ~ " << prob_chiSq<<std::endl;	
...
//   prelimTex->SetTextSize(0.07);
  prelimTex->SetTextSize(labelSize);
  prelimTex->SetLineWidth(2);
  prelimTex->DrawLatex(0.95,0.94,"P(#mu_{MR},#it{e}_{MR})^{1/10}");
...
//   c->SaveAs(outname+".C");
  ...
  for(int xbin=1; xbin <= h->GetNbinsX(); xbin++){
  	...
  	for(int ybin=1; ybin <= h->GetNbinsY(); ybin++){
		...
		//cout<< xbin << ybin << h->GetBinContent(xbin,ybin)<<endl;
  	} 
        //cout<< xbin << " " << tot_y<<endl;
  	hx->SetBinContent(xbin,tot_y);
  	//std::cout << "xbin = " << xbin << ", tot_y = " << tot_y <<std::endl;
  }
  //hx->GetXaxis()->SetTitle("#mu misidentification rate");
  hx->Fit("gaus","R","",10,20); //pay attention to the initial condition : range!
  //hx->Fit("gaus");
  hx->Draw();
  ...
  hy->Fit("gaus","R","",5,15); //pay attention to the initial condition: range !
  //hy->Fit("gaus");
  hy->Draw();
...
void DrawProbChiSqOneTenth(TFile *f,TString outname,Double_t xp, Double_t yp){
  TCanvas *c = new TCanvas("prob_chiSqOneTenth","prob_chiSqOneTenth",900,800);
  c->SetRightMargin(0.125);
  c->SetTopMargin(0.10);
  c->SetLeftMargin(0.10);
  c->SetRightMargin(0.15);
  ...
  TH2D *h_original = (TH2D*) f->Get("chiSq_sum");
  TH2D *h = (TH2D*) h_original->Clone();
  ...
  for(int xbin=1; xbin <= h->GetNbinsX(); xbin++){
        for(int ybin=1; ybin <= h->GetNbinsY(); ybin++){
                chiSq = h->GetBinContent(xbin,ybin);
                long double prob_chiSq  = exp( - (chiSq) / 20. );
                //std::cout << "chiSq (" << xbin<<", "<< ybin<< ") = " << chiSq << " --> prob ~ " << prob_chiSq<<std::endl;
                h->SetBinContent(xbin,ybin,prob_chiSq);
        }
  }
 ...
 c->SaveAs(outname+"_ProbabilityOneTenth.pdf");

}

void DrawConstChiSqContour(TString dir ="FRv1_kinematics_FWLJMET102X_3lep2017_062019_wiwong_step1hadds_step2FR_kinematics_FWLJMET102X_3lep2017_062019_wiwong_step1hadds_step2_2019_8_16"){

  TFile *f ;

  //enter fake rates here to draw                                                                                                                                                                        
  float muFR = 0.15;//0.16;//0.15;//0.14;//0.15 ;
  float elFR = 0.10;//0.12;//0.10;//0.10;//0.10 ;

//  TString brux_dir ="/user_data/rsyarif";
//  TString dir ="measureFR_LJMet94x_3lepTT_2017datasets_2018_11_7_rizki_FRCR2_2018_12_13";
//  TString filename = "chiSq_lepPt_41p557fb.root";
//  TString saveDir = "chiSq_plots";
  TString brux_dir ="/mnt/data/users/wwong";
//  TString dir ="FRv1_kinematics_FWLJMET102X_3lep2017_062019_wiwong_step1hadds_step2FR_kinematics_FWLJMET102X_3lep2017_062019_wiwong_step1hadds_step2_2019_8_16";
  TString filename = "chiSq_lepPt_41p557fb.root";
  TString saveDir = "chiSq_plots";
  ...
  DrawProbChiSqOneTenth(f,"chiSq_lepPt_"+dir,muFR,elFR);
