...
# outDir+='/optimization_condor_80x_MultiLep_Full2016_Moriond17_PRv6_FRv18bSys_fixedLumi_ALLsys_step2_addmistag_addMoreSignal_2017_2_17/lep1Pt0_jetPt0_MET20_NJets3_NBJets1_HT0_ST600_mllOS20/'
# newOutDir='/4binsCount_updatedLumi_updatedLepIdUnc_addmistag_normRENORMPDF'

outDir+='/optimization_condor_80x_MultiLep_Full2016_Moriond17_PRv6_FRv18bSys_fixedLumi_newMllOScut_fixedAllSYS_step2_addmistag_addMoreSignal_2017_2_23/lep1Pt0_jetPt0_MET20_NJets3_NBJets1_HT0_ST600_mllOS20/'
# newOutDir='/4binsCount_newMllOScut'

newOutDir='/4binsCount'
	...
	for BRind in range(nBRconf):
		...
		for signal in sigList:
				...
				for systematic in systematicList+normSystematics.keys()+ddbkgSystematics.keys():
					...
					for ud in ['Up','Down']:
						if 'muRFcorrdNew' in systematic:
							hewkY[signal+systematic+ud] = R.TH1F('triLep__ewk__'+systematic+'Ewk__'+ud.replace('Up','plus').replace('Down','minus'),'',len(catList),0,len(catList))
							htopY[signal+systematic+ud] = R.TH1F('triLep__top__'+systematic+'Top__'+ud.replace('Up','plus').replace('Down','minus'),'',len(catList),0,len(catList))
							hsigY[signal+systematic+ud] = R.TH1F('triLep__sig__'+systematic+'Sig__'+ud.replace('Up','plus').replace('Down','minus'),'',len(catList),0,len(catList))
			...
			if doAllSys:
				for systematic in systematicList+normSystematics.keys()+ddbkgSystematics.keys():
					for ud in ['Up','Down']:
						if not (systematic=='toppt' or 'PR' in systematic or 'FR' in systematic or systematic=='ddbkgSys'):
						...
						if systematic=='toppt': # top pt is only on the ttbar sample, so it needs special treatment!
							if htopY[signal+systematic+ud].Integral() > 0: htopY[signal+systematic+ud].Write()
						if ('PR' in systematic or 'FR' in systematic or systematic=='ddbkgSys'): # PR and FR is only on the ddbkg sample, so it needs special treatment!
