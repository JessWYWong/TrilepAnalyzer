...
#step1Dir_mc = '/mnt/data/users/wwong/FWLJMET102X_3lep2017_wywong_012020_step1_flatFRv4_TrigEff_uFR_hadds_step2'
#step1Dir_mc = '/mnt/data/users/wwong/FWLJMET102X_3lep2018_wywong_052020_step1_FRv1_uFr_hadds_step2'
#step1Dir_mc = '/mnt/data/users/wwong/FWLJMET102X_3lep2018_wywong_052020_step1_FRv1_TrigEff_hadds_step2'
#step1Dir_mc = '/mnt/data/users/wwong/FWLJMET102X_3lep2018_wywong_052020_step1_etaFRv2_DeepCSV_hadds_step2'
#step1Dir_mc = '/mnt/data/users/wwong/FWLJMET102X_3lep2018_wywong_052020_step1_FRv2_PRv2_hadds_step2'
...
if 'Eta' in cutType:
        cutList.update({'MinMuEta':0.0})
	cutList.update({'MuEtaCut':0.4}) #0.4,0.9,1.2,2.1,2.4

if len(sys.argv)>4: catList=[str(sys.argv[4])]
else: catList=['All','EEE','EEM','EMM','MMM']

MuEtaBins = [0.0,0.4]#[0.0,0.4,0.9,1.2,2.1,2.4]
...
if 'Eta' in cutType: cutString += '_MuEta[MinMuEta]_[MuEtaCut]'
...
HTbinsv2 = [0,100,200,300,400,500,600,800]
...
STbinsv3 = [0,100,200,300,400,500,600,700,800,900,1000,1100,1200]
...
DRJet12bins=linspace(0.0, 4.0, 13).tolist()+[5.5]
...
# 	'MuPt' :('AllLeptonMuPt_PtOrdered',linspace(0, 500, 26).tolist(),';Muon p_{T} (GeV);'),
        ...
        'DeepCSV' :('AK4JetDeepCSVb_MultiLepCalc_PtOrdered+AK4JetDeepCSVbb_MultiLepCalc_PtOrdered',linspace(0, 1, 31).tolist(),';DeepCSVb+bb;'),
        'DRJet12' :('deltaRJet1Jet2',DRJet12bins,';#DeltaR(jet_{1},jet_{2});'),
	...
	'Mu1Pt':('AllLeptonMuPt_PtOrdered[0]',pTbins,';Muon 1 p_{T} (GeV);'),
        'Mu1PtRebinned' :('AllLeptonMuPt_PtOrdered[0]',pTbinsV2,';Muon 1 p_{T} (GeV);'),
...
# 	'Jet2Pt':('AK4JetPt_MultiLepCalc_PtOrdered[1]',jetPtbins,';2^{nd} AK4 Jet p_{T} (GeV);'),
	...
	'HTrebinned'    :('AK4HT',HTbins,';H_{T} (GeV);'),
        'HTrebinnedv2'  :('AK4HT',HTbinsv2,';H_{T} (GeV);'),
...
LoopCuts = {}
if 'Eta' in cutType :
        for ind in range(1,len(MuEtaBins)):
		cutList['MuEtaCut'] = MuEtaBins[ind]
		cutList['MinMuEta'] = MuEtaBins[ind-1]
		cutStringTmp = cutString.replace("[MinMuEta]", str(float(cutList['MinMuEta'])).replace('.','p')).replace("[MuEtaCut]" , str(float(cutList['MuEtaCut'])).replace('.','p'))
		LoopCuts.update({cutStringTmp: cutList})
else:
	LoopCuts.update({cutString: cutList})


for category in catList:
	for icutString, icutList in LoopCuts.items():
		print category
		datahists = {}
		bkghists  = {}
		sighists  = {}
		...
		if not os.path.exists(outDir+'/'+icutString): os.system('mkdir '+outDir+'/'+icutString)
		outDir+='/'+icutString #'/'+cutString
		if not os.path.exists(outDir+'/'+category): os.system('mkdir '+outDir+'/'+category)
		outDir+='/'+category
		print 'outDir', outDir
		for data in dataList: 
			datahists.update(analyze(tTreeData,data,icutList,False,iPlot,plotList[iPlot],category,cutType))
			if catInd==nCats: del tFileData[data]
		for bkg in bkgList: 
			print '+++++++++++++++++++++>>>>>>>>>>>>>>>>>>>>>>>>>>  bkg = ' , bkg 
			bkghists.update(analyze(tTreeBkg,bkg,icutList,doAllSys,iPlot,plotList[iPlot],category,cutType))
		...
		for sig in sigList: 
			for decay in decays: 
				sighists.update(analyze(tTreeSig,sig+decay,icutList,doAllSys,iPlot,plotList[iPlot],category,cutType))
