...
for cut in allCuts.keys():
    ...
    for j in range(len(xbin)):
        ...
        for i in range(len(mass_str)):
            ...
            #if (expLimits[mass_str[i]][cutString]-expLimits[mass_str[i]][noCut])<0:
            #    if (expLimits[mass_str[i]][cutString]-expLimits[mass_str[i]][noCut])/expLimits[mass_str[i]][noCut] < maxDelta: maxDelta = (expLimits[mass_str[i]][cutString]-expLimits[mass_str[i]][noCut])/expLimits[mass_str[i]][noCut]
            #else:
            #    if maxDelta > 0 and (expLimits[mass_str[i]][cutString]-expLimits[mass_str[i]][noCut])/expLimits[mass_str[i]][noCut] > maxDelta: maxDelta = (expLimits[mass_str[i]][cutString]-expLimits[mass_str[i]][noCut])/expLimits[mass_str[i]][noCut]
